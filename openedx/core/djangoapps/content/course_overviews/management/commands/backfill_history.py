...
class Command(BaseCommand):
    ...
    TABLES = [
        {'name': 'organizations_organization', 'exclude_column': None, 'input_filename': 'organizations_organization_2019-06-29'},
        {'name': 'entitlements_courseentitlement', 'exclude_column': None, 'input_filename': 'entitlements_courseentitlement_2019-06-29'},
        {'name': 'course_overviews_courseoverview', 'exclude_column': None, 'input_filename': 'course_overviews_courseoverview_2019-07-11'}
    ]
        ...
        for table_info in self.TABLES:
            ...
            with open(file_path, 'r') as input_file:
                ...
                for rows in self.chunks(lines, batch_size):
                    ...
                    if count==len(rows):
                        log.info(u"Initial history records already exist for ids %s..%s - skipping.", ','.join(row_ids[:2]), ','.join(row_ids[-2:]))
                        continue
                    elif count!=0:
